...
class DocumentController extends Controller
{
    ...
    public function showUploadForm($collection_id)
    {
        ...
        return view('upload', [
            ...
            'document' => $document,
            'activePage' => 'Upload Document Form',
            'titlePage' => 'Upload Document',
        ...
        if ($request->hasFile('document')) {
            ...
            foreach ($documents as $file) {
                ...
                $filepath = $file->storeAs(
                    'smartarchive_assets/' . $request->input('collection_id') . '/' . \Auth::user()->id,
                    $new_filename,
                    $storage_drive
                );
            ...
            $d->path = count($filePaths) > 1 ? json_encode($filePaths) : $filePaths[0];
            $d->size = array_sum(array_map(fn($file) => $file->getSize(), $documents));
            $d->type = count($filePaths) > 1 ? 'multiple' : $documents[0]->getMimeType();

            if (!empty($request->input('title'))) {
                $d->title = htmlentities($request->input('title'));
            } else {
                $d->title = $this->autoDocumentTitle($documents[0]->getClientOriginalName());
            }
            ...
            try {
                $text_content = '';
                foreach ($filePaths as $local_filepath) {
                    $content = $this->extractText($local_filepath, $documents[0]->getMimeType());
                    $text_content .= mb_convert_encoding($content, "UTF-8") . "\n";
                }
                $d->text_content = $text_content;

                $d->save();
                $messages[] = 'Document(s) uploaded successfully!';
            } catch (\Exception $e) {
            ...
            // Logic for external link (as it is)
    ...
    private function validateUploadedData(Request $request)
    {
        ...
        $size_limit = ini_get("upload_max_filesize");
        $actual_size = $this->return_bytes($size_limit);

        foreach ($documents as $file) {
            $validator = Validator::make(['document' => $file], [
                'document' => 'file|max:' . $actual_size . '|mimes:' . env('FILE_EXTENSIONS_TO_UPLOAD', 'ppt,pptx,doc,docx,jpg,png,pdf,txt')
            ]);

            if ($validator->fails()) {
                return ['status' => 'failure', 'errors' => ['Invalid file: ' . $file->getClientOriginalName()]];
            }
        }
