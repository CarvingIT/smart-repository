                                    ...
                                    <tbody>
                                        @foreach ($sharedLinks as $link)
                                            <tr>
                                                ...
                                                <td>
                                                    <a href="{{ route('shared-links.edit', $link) }}" class="btn btn-sm btn-primary">{{ __('EDIT') }}</a>
                                                    <form action="{{ route('shared-links.destroy', $link) }}" method="POST" style="display: inline-block;">
                                                        @csrf
                                                        @method('DELETE')
                                                        <button type="submit" class="btn btn-sm btn-danger" onclick="return confirm('Are you sure you want to delete this shared link?')">{{ __('DELETE') }}</button>
                                                    </form>
                                                </td>
                                            </tr>
                                        @endforeach
