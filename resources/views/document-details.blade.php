            ...
            $(document).ready(function() {
                $("#wordcloud").jQWCloud({
                    ...
                    afterCloudRender: function() {
                        var date2 = new Date();
                        console.log("Cloud Completed in " + (date2.getTime() - date1.getTime()) +
                            " milliseconds");
                    }
                });
            });
                ...
                <div class="col-md-12">
                    <div class="card">
                        <div class="card-header card-header-primary">
                            <h4 class="card-title"><a href="/collections">{{ __('Collections') }}</a> :: <a
                                    href="/collection/{{ $c->id }}">{{ $c->name }}</a> :: Document Details</h4>
                        </div>
                        <div class="card-body">

                            <div class="row">
                                <div class="col-md-12 text-right">
                                    <a href="#" onclick="$('#related_document_form').show(); return false;"
                                        class="btn btn-sm btn-primary" title="Related Documents">
                            ...
                            <div class="card-body">
                                ...
                                <div id="related_document_form" style="display:none;">
                                    ...
                                    <form method="post"
                                        action="/collection/{{ $document->collection->id }}/document/{{ $document->id }}/add-related-document">
                                        @csrf
                                        <input type="hidden" id="document_id" name="document_id"
                                            value="{{ $document->id }}" />
                                        <input type="hidden" id="related_document_id" name="related_document_id"
                                            value="" />
                                        <div class="row">
                                            <div class="col-md-2">
                                                <label for="title-autocomplete"
                                                    class="col-md-12 col-form-label text-md-right">Search title</label>
                                            </div>
                                            <div class="col-md-8">
                                                <input class="form-control" type="text" id="title-autocomplete"
                                        ...
                                        <div class="row">
                                            <div class="col-md-2">
                                                <label for="display_order"
                                                    class="col-md-12 col-form-label text-md-right">Display order</label>
                                            </div>
                                            <div class="col-md-8">
                                                <input class="form-control" type="number" id="display_order"
                                                    name="display_order" />
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-2">
                                                <label for="title_override"
                                                    class="col-md-12 col-form-label text-md-right">Override title</label>
                                            </div>
                                            <div class="col-md-8">
                                                <input class="form-control" type="text" id="title_override"
                                                    name="title" />
                                            </div>
                                        </div>
                                        <div class="row">
                                            <div class="col-md-12 text-center">
                                                ...
                                                <input class="btn" type="button" value="Cancel"
                                                    onclick="$('#related_document_form').hide();" />
                                            </div>
                                        </div>
                                    </form>
                                    <br />
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        <span id="doc-title" class="col-md-12">
                                            @if ($c->content_type == 'Uploaded documents')
                                                @if ($document->type == 'array') <!-- Check if it's an array of files -->
                                                    @php
                                                        $filePaths = json_decode($document->path); // Decode the JSON array
                                                    @endphp

                                                    @if (!empty($filePaths)) <!-- If there are files -->
                                                        @foreach ($filePaths as $filePath)
                                                            <a href="{{ route('documents.getFile', ['collectionId' => $collection->id, 'documentId' => $document->id, 'filename' => basename($filePath)]) }}" target="_new">
                                                                <img class="file-icon" src="/i/file-types/{{ $document->icon($filePath) }}.png" style="float:left;">
                                                                {{ basename($filePath) }}
                                                            </a>
                                                            <br>
                                                        @endforeach
                                                ...
                                                @elseif($document->type == 'application/pdf')
                                                    @if (env('ENABLE_PDF_READER') == 1)
                                                        <a href="/collection/{{ $c->id }}/document/{{ $document->id }}/pdf-reader" target="_new">
                                                    ...
                                                    @else
                                                        <a href="/collection/{{ $c->id }}/document/{{ $document->id }}" target="_new">
                                                            <img class="file-icon" src="/i/file-types/{{ $document->icon($document->path) }}.png" style="float:left;">
                                                        </a>&nbsp;
                                                ...
                                                @elseif($document->type == 'application/vnd.openxmlformats-officedocument.presentationml.presentation')
                                                    <a href="/collection/{{ $c->id }}/document/{{ $document->id }}">
                                                        <img class="file-icon" src="/i/file-types/{{ $document->icon($document->path) }}.png" style="float:left;">
                                                    </a>&nbsp;<a href="/collection/{{ $c->id }}/document/{{ $document->id }}">
                                                @elseif(preg_match('/^audio/', $document->type) || preg_match('/^video/', $document->type))
                                                    <div style="text-align:center;">
                                                        <h3>
                                                            <a href="/collection/{{ $c->id }}/document/{{ $document->id }}">
                                                                <img class="file-icon" src="/i/file-types/{{ $document->icon($document->path) }}.png">
                                                            </a>
                                                            {{ $document->title }}
                                                        </h3>
                                                        <video controls>
                                                ...
                                                @else
                                                    @if ($document->path != 'N/A')
                                                        <a href="/collection/{{ $c->id }}/document/{{ $document->id }}">
                                                            <img class="file-icon" src="/i/file-types/{{ $document->icon($document->path) }}.png" style="float:left;">
                                                        </a>&nbsp;<a href="/collection/{{ $c->id }}/document/{{ $document->id }}" target="_new" style="text-decoration:underline;">
                                                    @else
                                                        <img class="file-icon" src="/i/file-types/{{ $document->icon($document->path) }}.png" style="float:left;" />
                                                    @endif
                                                @endif
                                    ...
                                    @if ($c->content_type == 'Uploaded documents')
                                        ...
                                        @foreach ($document->collection->meta_fields as $meta_field)
                                            @php
                                                $m = App\MetaFieldValue::where('document_id', $document->id)
                                                    ->where('meta_field_id', $meta_field->id)
                                                    ->first();
                                                if (!$m) {
                                                    continue;
                                                }
                                            ...
                                            @if (!empty($meta_labels[$m->meta_field_id]))
                                                @php
                                                    $extra_attributes = empty($m->meta_field->extra_attributes)
                                                        ? null
                                                        : json_decode($m->meta_field->extra_attributes);
                                                    $w = empty($extra_attributes->width_on_info_page)
                                                        ? 12
                                                        : $extra_attributes->width_on_info_page;
                                                    $show_on_details_page = empty(
                                                        $extra_attributes->show_on_details_page
                                                    )
                                                        ? ''
                                                        : $extra_attributes->show_on_details_page;
                                                    $classname = @$extra_attributes->results_classname;
                                                    if ($show_on_details_page == 0) {
                                                        continue;
                                                    }
                                                @endphp
                                                <div class="col-md-{{ $w }}">

                                                    <label for="doc-meta-{{ $meta_labels[$m->meta_field_id] }}"
                                                        class="col-md-12">
                                                        <div class="{{ $classname }}">&nbsp;</div>
                                                        {{ $meta_labels[$m->meta_field_id] }}
                                                    </label>
                                                    @if ($m->meta_field->type == 'MultiSelect' || $m->meta_field->type == 'Select')
                                                        <span id="doc-meta-{{ $meta_labels[$m->meta_field_id] }}"
                                                            class="col-md-12">{{ @implode(', ', json_decode($m->value)) }}</span>
                                                    @elseif ($m->meta_field->type == 'TaxonomyTree')
                                                        <span id="doc-meta-{{ $meta_labels[$m->meta_field_id] }}"
                                                            class="col-md-12">{{ $document->meta_value($m->meta_field_id) }}</span>
                                                    @else
                                                        <div id="doc-meta-{{ $meta_labels[$m->meta_field_id] }}"
                                                            class="col-md-12">{!! html_entity_decode($m->value) !!}</div>
                                                    @endif
                                                </div>
                                                <br />
                                            @endif
                                                ...
                                                @if (\Auth::user()->hasPermission($document->collection_id, 'APPROVE'))
                                                    <form name="doc_approve" method="post" action="/approve-document">
                                                        ...
                                                        @if (!empty($document->approved_on))
                                                            ...
                                                            <input id="approved_on" type="hidden" name="approved_on"
                                                                value="" />
                                            ...
                                            @if (empty($document->approved_on))
                                                <div class="col-md-12">
                                                    ...
                                                    @foreach ($comments as $comment)
                                                        <div>{{ $comment->user->name }}({{ $comment->user->email }}) said:
                                                            <p>{{ $comment->comment }}</p>
                                                        </div>
                                                    @endforeach
                                                </div>
                                                <div class="col-md-12">
                                                    <form name="comment" action="/save-comment" method="post">
                                                        ...
                                                        <div id="doc_meta_description" class="col-md-12">
                                                            <textarea name="comment" id="comment" class="form-control" cols="80" rows="10" value=""
                                    ...
                                    <div class="col-md-3">
                                        ...
                                        <ul>
                                            @foreach ($document->related_documents as $r_d)
                                                <li><a
                                                        href="/collection/{{ $r_d->related_document->collection->id }}/document/{{ $r_d->related_document->id }}/details">{{ $r_d->related_document->title }}</a>
                                                </li>
                                            @endforeach
                                        </ul>
                                    </div>
                            ...
                            <div id="deletedialog" style="display:none;">
                                <form name="deletedoc" method="post" action="/document/delete">
                                    ...
                                    <input type="hidden" id="delete_doc_id" name="document_id"
                                        value="{{ $document->id }}" />
                            ...
                            <div class="row">
                                <div class="col-md-12">
                                    @if (Auth::user() && Auth::user()->canEditDocument($document->id))
                                        <a href="/document/{{ $document->id }}/edit" class="btn btn-sm btn-primary"
                                            title="Edit">
                                            <i class="material-icons">edit</i>
                                        </a>
                                    @endif
                                    @if (Auth::user() && Auth::user()->canDeleteDocument($document->id))
                                        <a href="javascript:return false;" onclick="showDeleteDialog();"
                                            class="btn btn-sm btn-primary" title="Delete">
                            ...
                            @if (!empty($col_config->show_word_cloud))
                                <div class="col-md-12">
                                    <div id="wordcloud"><img src='/i/processing.gif'></div>
                                </div>
                            @endif
